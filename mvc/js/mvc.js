...
!function(){
    ...
    var fn = Fw.apply(controller.prototype, Fw.Event ,{
        ...
        listeners : function(){//负责自定义事件监听
            ...
            me.on('list:render', function () {
                me.view.render(me.model.getPageData());
            });
            
            me.on('list:show', function () {
                this.roles['list-wrap'].classList.remove('hide');
            });

            me.on('list:hide', function () {
                this.roles['list-wrap'].classList.add('hide');
            });

            me.on('list:select', function (ev) {
                try{
                    var target = ev.data.target;
                    var data = target.showHtml;

                    this.roles['input'].value = data.filed;
                    this.roles['input'].dataset.value = data.value;
                    this.selectData = data;
                }catch(e){
                    throw  new Error('事件触发元素不对！')
                }
            });

            me.on('change:pageIndex',function(){
                me.fire('list:render');
                me.roles['page-input'].value = me.pageIndex;
            });
        },
        ...
        click : {
            ...
            'list' : function (ev) {
                this.fire('list:select', {target : ev.target});
                this.fire('list:hide');
            },
            'page-pre' : function () {
                this.pageIndex --;
                this.fire('change:pageIndex');
            },
            'page-next' : function () {
                this.pageIndex ++;
                this.fire('change:pageIndex');
        ...
        keyup : {
            'page-input': function (ev) {
                try{
                    var val = ev.target.value;

                    if(/^[1-9][0-9]{0,5}$/g.test(val+'')
                        && val != this.pageIndex){
    ...
    fn.model = {//模型数据
        ...
        getPageData : function(){
            var start = (this.heart.pageIndex - 1)*this.heart.pageSize;
            var end = this.heart.pageIndex*this.heart.pageSize;
    ...
    fn.view = {//视图
        ...
        '<ul class="combox-list" data-role="list" data-event="click:list"></ul>' +
        '<div class="combox-page" data-role="page-wrap">' +
        '<span class="combox-page-pre" data-role="page-pre" data-event="click:page-pre"><<</span>' +
        '<input class="combox-page-input" maxlength="6" data-role="page-input" value="1" data-event="keyup:page-input">' +
        '<span class="combox-page-next" data-role="pageNext" data-event="click:page-next">>></span></div></div></div>',
                    ...
                    Object.defineProperty(node, 'showHtml',{
                        ...
                        get : function(){
                            if(this.classList.contains('hide')){
                                ...
                                return {filed : this.innerText, value : this.dataset.value};
                            }
                        }
                    });
